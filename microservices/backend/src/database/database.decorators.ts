...
import { Constructor, Decorator } from '@root/defs';

import { RelationArgs } from './database.defs';
...
export function getRelations<T>(entity: Constructor<T>) {
  return Reflect.getMetadata(RELATIONS_METADATA_KEY, entity) as RelationArgs<
    any,
    any
  >[];
}
